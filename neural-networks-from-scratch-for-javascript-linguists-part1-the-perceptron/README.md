...
*Перевод статьи [Elyx0](https://hackernoon.com/@Elyx0): [Neural networks from scratch for Javascript linguists (Part1 — The Perceptron)](https://hackernoon.com/neural-networks-from-scratch-for-javascript-linguists-part1-the-perceptron-632a4d1fbad2).*
...
Не парьтесь, у меня та же история, так что давайте отправимся в **путешествие**. Я расскажу вам все, что узнал, некоторые мои заблуждения, способ правильно интерпретировать результаты и некоторые основные словарные и интересные факты на этом пути.
...
Изначально ваша коробка **тупая** как картон, она не выдаст магическим образом результат, которого вы ожидаете. Вы должны **обучить** ее достижению **цели**, которую вы хотите.
...
Некоторые функции активации могут давать отрицательные величины, а некоторые - нет. Это будет иметь важное значение при использовании результатов функции активации одного перцептрона для подачи в качестве входов другого перцептрона.
Использование `0` в качестве входного сигнала исключает связанный с ним вес из конечной суммы.
Таким образом, вместо наличия возможности получения отрицательного веса, `0` действует как деактиватор. В то время как нам может потребоваться этот вес для инверсии значимости входа в итоговой сумме.

![Существуют различные функции активации](https://cdn-images-1.medium.com/max/800/1*dq-QWOg88oDkgMKBeCujTg.png)

Перцептрон известен как **бинарный классификатор**. Это означает, что он может классифицировать только на **2 класса** (Спам или не спам, Апельсин или не апельсин... и т. д.).
...
`Y = mx + c`, или `y = ax + b`
Как же мы получаем эти `m` и `c` из нашего тренированного перцептрона?
...
Знак **минус** перед `w1` означает, что если оба веса имеют один и тот же знак, линия будет наклонена вниз, как `\`, если знаки отличаются друг от друга, она будет направлена вверх `/`.
Регулировка **w1** будет влиять на угол поворота линии, но не на координату точки пересечения с вертикальной осью. вместо этого **w2** будет влиять и на **угол поворота и на координату точки пересечения**. Поскольку **сдвиг (bias) - это числитель** (верхняя половина фракции), его увеличение поднимает линию выше по графику. (точка пересечения с вертикальной осью будет выше)
...
Один из способов понять это - представить [парня в железных сапогах](https://i.ytimg.com/vi/qHuhZxyWiFA/hqdefault.jpg), связанных вместе, который хочет попасть к сокровищу на дне скалы, но может двигаться только прыжком на **α** метров:
...
Кроме того, Перцептрон относится к категории **нейронных сетей прямого распространения**, которая является просто причудливой формулировкой, говорящей о том, что связи между элементами не образуют цикл.
