...
Хорошо, на самом деле сообщает Babel: «Эй, скомпилируй этот jsx, но вместо `React.createElement`, подставь `h`». Здесь вы можете подставить что угодно вместо `h`. И это будет скомпилированно.

Итак, подводя итоги того, что я сказал раньше, мы будем писать наш DOM таким образом:
...
Ок, давайте преположим что index будет передоваться нашей функции (и это действительно произойдет - вы увидите позже). Так наш код будет таким:
```
...
Напишем функцию, которая сравнивает 2 узла и сообщает нам, если узел действительно изменился. Мы должны учитывать, что это могут быть как элементы так и текстовые узлы:
...
Но есть несколько вещей, которые необходимо рассмотреть, прежде чем писать код:
* Мы должны сравнивать детей только если это элементы (строки не могут иметь детей)
* Сейчас мы должны передовать ссылку на текущей узел как на родителя.
* Мы должны сравнить всех детей один за одним - даже если в какой-то моменты мы получим undefined  - это ок - наша функция должна обработать это.
* И, наконец, index - это просто указатель на дочерний элемент 
...
## Собираем все вместе
Да, это он. Мы там. Я поместил весь код в [JSFiddle](https://jsfiddle.net/deathmood/0htedLra/?utm_source=website&utm_medium=embed&utm_campaign=0htedLra) и реализации действительно составляет 50 LOC — как я вам и обещал.  Теперь можете пойти и поиграть с ним.

Откройте инструменты разработчика и посмотрете изменения при нажатии кнопки `reload`.
