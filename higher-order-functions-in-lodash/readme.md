...
> Частичное применение — процесс фиксации части аргументов функции, создающий другую функцию, меньшей арности.
...
После преобразования функции с помощью `_.curry()` мы получаем совершенно новую функцию, накапливающую и фиксирующую параметры для последующих вызовов, пока все ожидаемые параметры не будут указаны и исходная функция не будет вызвана. Параметры могут быть указаны один за другим или по несколько сразу. Как вы видите, вы можете также пропускать параметры, используя заместитель `_`, так же как и в `_.partial()`. 
