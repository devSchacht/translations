...
Однако, если мы хотим придерживаться ES 5, мы можем переписать функцию, используя возможности Lodash. Lodash принимает arguments как параметр функции везде, где ожидается массив в качестве параметра. Вы также можете легко конвертировать любой похожий на массив объект к настоящему массиву с использованием _.toArray(). Наша улучшенная реализация будет выглядеть так: 
...
Lodash предлагает еще одну функцию, похожую на _.partial(). Эта функция называется _.curry(). Давайте попробуем ее: 
