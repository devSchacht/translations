...
Вероятно, вы знаете, что стандартный Ajax-запрос не завершается синхронно. Это значит, что функция `ajax(...)`, еще не имеет никакого значение, которое можно было бы вернуть и присвоить переменной `data`. Если `ajax(...)` можно заблокировать, до тех пор пока ответ не вернется, то присваивание `data = ...` будет работать нормально.

Но Ajax-запросы так не применяются. Мы выполняем асинхронный Ajax-запрос сейчас, и мы не получим данные *потом*.

Простейший (но не единственный и не обязательно самый лучший) способ "подождать" от *сейчас* до *потом*, является использование колбэков:
...
Часть *сейчас* запустится сразу, по ходу выполнения программы. Но `setTimeout()` добавляется новое событие (задержку), которое выполнится *потом*, поэтому содержимое функции `later()` будет выполнено позднее (через 1000 миллисекунд).
...
Нет спецификации или требований, описывающих работу `console.*` методов — они не являются официальной частью JavaScript. Вместо этого они добавлены в JS с помощью среды хостинга.
...
Другими словами, JS-движок не имел врожденного чувства *времени*, зато имел специфическую среду выполнения по требованию любого произвольного фрагмента кода. В этой среде существовало некое "расписание" *событий* (выполнения JS-кода).
...
Поскольку два или более «процесса» чередуют свои шаги / события одновременно в рамках одной и той же программы, они не обязательно должны взаимодействовать друг с другом, если задачи, которые они выполняют, не связаны друг с другом. Если они не взаимодействуют, то такая **неопределенность** вполне приемлема.
...
Предположим, что ожидаемое поведение заключается в том, что `res[0]` имеет результат обработки «http: //some.url.1», а `res[1]` имеет результат обработки «http: //some.url.2». Да, такое поведение иммет место быть, но возможна и обратная ситуация. Все зависит от того, какой метод завершится первым. Налицо «состояние гонки».

**Примечание:** Будьте предельно осторожны в предположениях, которые вы могли бы сделать в этих ситуациях. Например, разработчик нередко замечает, что «http: //some.url.2» «всегда» гораздо медленнее выполняется, чем «http: //some.url.1». Например один метод выполняет работу с базой данных, а другой просто извлекает статический файл. Может возникнуть иллюзия очевидности порядка выполнения методов. Даже если оба запроса переходят на один и тот же сервер и выполняются в определенном порядке, нет никакой гарантии того, какой порядок ответов будет возвращен.
...
Те же наблюдения из этого сценария будут применяться, если несколько параллельных вызовов функций взаимодействуют друг с другом через общий DOM. Например, один обновляет содержимое `<div>`, а другой обновляет стиль или атрибуты `<div>` (например, сделать элемент видимым, если тот имеет контент). Вероятно, вы не захотите показывать элемент до того, как он будет иметь контент, поэтому кнеобходимо обеспечить правильный порядок взаимодействий.
...
В этом примере нет разницы, какой из методов будет вызван первым: `foo()` или `bar()`. В обоих случаях вызов `baz()`  произойдет слишком рано (либо `a`, либо `b` будут неопрделены). Но второй вызов `baz()` будет работать, поскольку и `a` и `b` будут доступны.
...
Мы можем координировать взаимодействие с помощью простой "защелки", чтобы позволить выполнияться только одному методу:
...
Условие `if (a == undefined)` вызов только из  `foo()` или `bar()`. Второй (и даже любой последующий) вызов будет игнорироваться.

**Примечание:** Недостатком вышеперечисленных методов координации является использоване глобальных переменных. Пока функциям будут доступны, то все проверки будут выполняться, как надо. В следующих главах мы рассмотрим более совершенные способы кооординаций.

### Кооперация

Другим приемом координации является «совместным параллелизмом». Здесь основное внимание уделяется не взаимодействию с глобальными переменными (хотя это, очевидно, все еще разрешено!). Прием состоит в том, чтобы выполнить длительный «процесс» и разбить его на этапы, чтобы другие параллельные «процессы» имели возможность чередовать свои операции в очереди цикла событий.

Расмотрим обработчик Ajax-ответа, который перебрать длинный список результатов, чтобы преобразовать значения. Мы будем использовать `Array# map(..)`, чтобы сократить код:
...
function response(data) {
	...
	if (data.length > 0) {
		// асинхронный запуск обработси следующей части результотов
		setTimeout( function(){
			response( data );
		}, 0 );
	}
...
Вы можете предположить, что результат будет `A B C D`, но вместо этого получите `A C D B`, потому что "Задания" выполнятся в конце текущего цикла цикла события, а выполнения таймера будет запоанировано на следующий тик цикла события (если он доступен!).
...
Порядок, в котором мы выражаем опереторы в нашем коде, не обязательно повторится в JS-движке. Это может показаться довольно странным утверждением, поэтому мы просто кратко рассмотрим его.
...
Однако, существует *вероятность*, что в процессе компиляции, движок может найти более быстрый способ выполнения вашего кода. Достигается это путем перестановки (безапасной) порядка операторов.
...
Другие примеры, когда переупорядочение при компиляции может создавать наблюдаемые сайд-эффекты (и, следовательно, должны быть запрещено), включая такие вещи, как вызов функций с сайд-эффектами (особенно геттров) или объекты ES6 Proxy:
...
Если бы не `console.log(..)` в этом фрагменте (тспольхуюущийся, как удобная форма наблюдения сайд-эффекта), JS-движок, вероятно, был бы волен использовать любые оптимизации, чтобы изменить порядок кода:
...
Хотя семантика JS, к счастью, защищает нас от наблюдаемых кошмаров, которые, по-видимому, представляют опасность для переупорядочения операторов компилятора, по-прежнему важно понять, насколько тонкая связь существует между тем, как создается исходный код (сверху вниз) и то, как он запускается после компиляции.


Переупорядочение операторов при компиляции — это почти микро-метафора для параллелизма и взаимодействия. Она может помочь вам лучше понять проблемы с асинхронным JS-кодом.

### Итоги:

Программа JavaScript практически всегда разбивается на два или более фрагментов. Сначала выполняется первый фрагмент, а следующий фрагмент запускается позже, в ответ на на выполнение первого. Несмотря на то, что программа выполняется по отдельности, все фрагменты имеют одинаковый доступ к области видимости и состоянию программы, поэтому каждая модификация состояния выполняется поверх предыдущего состояния.
...
Часто бывает необходимо выполнить некоторые коррдинации взаимодействия между этими параллельными «процессами». Например, для обеспечения определенного порядка выполнения или предотвращения «состояния гонки». Эти «процессы» также могут разбиваться на более мелкие куски и позволяя другим «процессам» чередоваться.
