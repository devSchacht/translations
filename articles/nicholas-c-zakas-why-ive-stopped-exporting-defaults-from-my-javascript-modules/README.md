...
После нескольких лет борьбы с дефолтными экспортами, я переосмыслил свои подходы.

На прошлой неделе я написал в Твиттере кое-то, что получило немало неожиданных ответов.

Я написал это после того как понял, что большое количество моих проблем с JavaScript модулями могут быть связанны с дефолтными экспортами. И нет разницы, использовал ли я JavaScript модули (так называемые ECMAScript модули) или CommonJS модули, я все еще спотыкался об импорты из модулей с дефолтными экспортами. Я получил множество ответов на твит, многие из которых спрашивали как я пришел к такому решению. Этот пост — попытка прояснить мои мысли.
...
-   Тот случай, когда непонятно чем является экспорт, функцией или классом. _Это и не только_ решают именованные экспорты.
-   С этой проблемой сталкиваешься не только в своих проектах, но и в библиотеках, автором которых не являешься. Это значит, что соглашения об именовании файлоов не решают все прооблемы.
...
> _ES6 поддерживает стиль одиночного/дефолтного экспорта и предоставляет приятных синтаксис для дефолтного экспорта._
>
> — Дэвид Херман (David Herman) [19 июня 2014](https://mail.mozilla.org/pipermail/es-discuss/2014-June/037905.html)

Поэтому, в JavaScript модулях можно использовать дефолтныый экспорт:
...
Еще раз, `LinkedList` это произвольный выбор (если не аргументированныйы), он может быть как `Dog`, так и `simphony`.
...
1. **Явно лучше чем неявно.** Я не люблю код с секретами.
2. **Имена должны быть консистенты во всех файлах.** Если что-то является яблоком (`Apple`) в одном файле, то я не должен называть это апельсином (`Orange`) в другом файле. Яблоко (`Apple`) всегда должно оставаться яблоком (`Apple`).
...
В обоих случаях, это становится дополнительной информацией, которую необходимо запомнить, чтобы избежать повторной «прогулки по коду», когда потребуется опять импортировать из `list.js`. Если вы импортируете много дефолтных экспортов из ммодулей, тогда или ваша когнитивная нагрузка, или количество «прогулок по коду» увеличивается. Оба варианта не оптимальны и могут быть неприятными.
