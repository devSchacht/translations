...
Хотя многим разработчикам, не одобряющим фреймворки, эта идея может понравится, помните, что **делая ваш основной код зависимым от множества библиотек**, придется терпеть их особенности или решения мейнтейнеров (например несовместимость API в следующем релизе...). И стоит учитывать, что библиотеки разрабатываются независимо друг от друга (и от React), делая поддержку кода с годами всё сложнее.
...
Документация - один из важнейших аспектов для фреймворка или библиотеки. Она дает возможность пользователям использовать платформу по максимуму и, конечно, хорошая документация мотивирует проверять сначала её, прежде чем искать ответы в Google.
