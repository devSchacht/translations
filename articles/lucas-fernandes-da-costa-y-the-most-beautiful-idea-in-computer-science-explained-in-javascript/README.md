...
Это означает, что он будет продолжать вызывтаь `factorialRecursive(factorialRecursive)` (который является, по сути, `factorial`) пока `n` не достигет `0`, потом вернёт `1` и позволит нам вычислить факториал числа. Примерно так:
