...
В отличие от классов, `interface` представляет собой виртуальную структуру, которая существует только в контексте TypeScript. Компилятор TypeScript использует интерфейсы исключительно для целей проверки типов. Как только код будет преобразован в JavaScript, он будет очищен от интерфейсов - в JavaScript нет типов, поэтому для них там нет места.
