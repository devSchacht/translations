...
Для реализации поддержки директивы `v-model` необходимо реализовать событие `event`. Но как насчёт других событий, таких как события нажатий с мыши и клавиатуры и т.д.? В то время как нативные события всплывают в HTML (DOM), обработчики событий во Vue по умолчанию не делают это.
