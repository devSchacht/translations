...
Благодаря [недавнему выпуску](https://github.com/vuejs/vue-cli/blob/dev/CHANGELOG.md) стабильного релиза утилиты [Vue CLI](https://cli.vuejs.org/) сейчас самое время взглянуть на некоторые из её новых возможностей. Благодаря полной переработке на основе отзывов, полученных от растущего сообщества разработчиков [Vue.js](https://ru.vuejs.org/index.html), эта версия предоставляет новый набор инструментов, ориентированных на улучшение процесса разработки.
...
> Примечание переводчика: если версия утилиты `vue-cli` является устаревшей (например — `3.0.0-beta.15`), ее можно обновить до последней версии точно такими же командами, фактически переустановив утилиту в системе.
...
![The “Home Page” of Vue UI](./images/vue-ui-01.png)
...
![The basic details of your new project](./images/vue-ui-04.png)
...
![Presets Selection](./images/vue-ui-05.png)
...
![Selecting Features](./images/vue-ui-06.png)

На вкладке «Параметры» («Features») можно настроить различные параметры будущего проекта — от использования [Babel](https://babeljs.io/) и [TypeScript](https://www.typescriptlang.org/) до включения дополнительных компонентов Vue, таких как [Vuex](https://vuex.vuejs.org/ru/) или [Vue Router](https://router.vuejs.org/ru/).
...
![Saving your choices as a preset for later](./images/vue-ui-08.png)
...
![The default plugins for your new project](./images/vue-ui-09.png)
...
![Committing to Git](./images/vue-ui-12.png)
...
![Vue i18n is now enabled; a new sub menu item is available in the left panel](./images/vue-ui-13.png)
...
Еще одна очень полезная функция Vue UI — возможность запускать и диагностировать задачи (`tasks`) проектов (по другому называемые как `npm`-скрипты) непосредственно из пользовательского интерфейса Vue UI.
...
![The Analyzer tab showing the impact of each module within the application](./images/vue-ui-16.png)

## Локализация приложения

Ранее в этом уроке мы установили плагин [vue-i18n](https://github.com/kazupon/vue-i18n) через интерфейс управления Vue UI.

Когда мы это сделали, то боковом меню Vue UI автоматически был добавлен дополнительный элемент (_значок глобуса_), представляющий собой плагин `vue-i18n`.

Если выбрать этот значок, то откроется окно «Локализации проекта» («Project localizations») с графическим интерфейсов для настройки данного плагина. Здесь можно задать статический текстовый контент и соответствующую ему локализацию (_перевод этого контента_) на любых выбранных вами языках:
...
Чтобы приступить к локализации статического текстового контента приложения, сначала необходимо _добавить дополнительные языки_. Для этого нажмите кнопку «Добавить локаль» («Add locale»):
...
![Translating text content for the newly added locale](./images/vue-ui-19.png)

Если посмотреть на исходный код приложения в редакторе [Visual Studio Code](https://code.visualstudio.com/), то увидим, что интерфейс локализации автоматически преобразовался в соответствующий исходный код:
