...
Очень важным аспектом любого языка программирования является его система типов и типы данных в нем. Для строго типизированных языков программирования, например для таких как Java, переменные определяются конкретными типами, которые в свою очередь ограничивают значения переменных.

> Несмотря на то, что JavaScript является динамически типизированным языком программирования, существуют расширения над языком, которые поддерживают строгую типизацию, например [TypeScript](https://www.typescriptlang.org/).

С JavaScript возможно иметь переменную, которая была определена как содержащая значение типа `string`, а несколько позже на пути своего жизненного цикла она становится ссылкой на значение типа  `object`. Также случается, что JavaScript движок неявно занимается приведением типов во время выполнения сценария. Проверка типов очень важна для написания предсказуемых JavaScript программ.

> Для проверки типов в JavaScript присутствует довольно простой оператор [typeof](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof)
...
Однако в ES6, переменные объявленные в блочной области видимость с помощью `let` и `const` будут возвращать `ReferenceError` , если они использовались с оператором `typeof` до того как были инициализированы. А все потому что:


> Переменные имеющие блочную область видимости остаются во [временной мертвой зоне](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/let#Temporal_Dead_Zone) до момента инициализации:


```js
// Использование typeof для идентификатора объявленного в блочной области видимости момент создаст ReferenceError ошибку
...
| host objects | зависит от имплементации |
...
- проверка класса объекта с помощью `toString()` метода объект
...
Очень важным здесь является использование оператора строго сравнения. Следующий фрагмент кода иллюстрирует использования `undefined` значения:
...
`NaN` является специальным значением получаемым в результате арифметических операций, когда нет определения как можно представить результат. Например `(0 / 0) => NaN`. Также когда совершена попытка преобразования не численного значения, у которого отсутствует возможное численного представление, результатом преобразования будет `NaN`.
...
Функция выше очень похожа на имплементацию функции `Number.isNaN()`, добавленную в ES6, и следовательно, ее можно использовать в качестве polyfill для сред выполнения отличной от ES6, следующим образом:
...
В заключении вы можете усилить проверку с помощью `Object.is()` добавленной в ES6 для проверки на значение `NaN` .Функция `Object.is()` позволяет проверить что два переданных в нее значения это одно и тоже значение:
...
// Unexpected results within frames @TODO
function isArray(value) {
...
Фрагмент кода ниже демонстрирует результаты проверки типов с использованием созданной `type()` функции:
...
## Бонус: все не является объектами
...
Это ошибочное утверждение и на самом деле это неправда. Все в JavaScript не является объектом. Примитивы не являются объектами.
...
Если вы заинтересованы в получении дополнительной информации об операторе typeof в Javascript, обратитесь к [этой статье](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/typeof).
