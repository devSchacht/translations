...
Функция выше очень похожа на реализацию функции `Number.isNaN()`, добавленную в ES6, и следовательно, ее можно использовать в качестве полифил для сред выполнения, отличных от ES6, следующим образом:
...
В заключение вы можете усилить проверку с помощью `Object.is()`, добавленную в ES6 для проверки на значение `NaN`. Функция `Object.is()` позволяет проверить, что два переданных в нее значения это одно и тоже значение:
