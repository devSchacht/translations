...
Но представьте, что у нас гораздо больше регулярных выражений для парсинга, а не только `parseSsn` и `parsePhone`. Каждый раз, когда мы создаём парсер регулярным выражением, мы должны помнить о том, чтобы добавить `.exec` в конце. И уж поверьте мне, это легко забыть.
