...
Обратите внимание, как типизация встроенна в само определение функции. Всё становится ещё туманнее, когда у вас появляются обобщения типов данных (*прим. пер., дженерики*):
...
Я отметил жирным шрифтом типы, которые выделяют их, но они всё равно продолжают запутывать опеределение функции. Вы должны читать аккуратно, чтобы найти имена переменных.
...
Гораздо лечге чиать код без всякой надоедливой информации о типах. Единственная проблема в том, что мы теряем безопасность типизации. Мы можем просто передать параметры в обратном направлении, то есть *Number* для `people` и *Object* для `personId`.
...
Ситуация приобретёт для вас больше смысла, когда вы увидете аннотацию с подразумевыемыми круглыми скобками:
...
Если вы видете `(a -> a)`, значит имеется ввиду, что входящий и выходящий типы ОБЯЗАНЫ быть одинаковыми. Не имеет значения, какие они именно, главное, чтобы они соответствовали друг другу.
...
Тип `List Int` означает список, содержащий типы `Int`, и `List String` означает список, содержащий `String`. Если вы используете обобщения типов даных в Java или других языках, тогда такая концепция должна быть вам близка.
