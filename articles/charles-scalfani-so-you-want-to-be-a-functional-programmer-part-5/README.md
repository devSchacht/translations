...
Большинство программ однопоточны, то есть одна и только одна часть кода выполняется за определённый промежуток времени. Даже если у вас многопоточная программа, большинство потоков блокируется ожиданием выполнения процессов ввода-вывода, например, загрузкой файла, ответом сети и так далее.
...
Но, как только мы это делаем, всё сильно усложняется:
...
Несмотря на то, что это звучит ненормально, идея не настолько хаотична, насколько может показаться сначала. Давайте взглянем на некоторый код Elm, чтобы проиллюстрировать это:
...
Во-вторых, следующей истиной должно быть условие, при котором результат выполнения одной функции не является входным значением другой. Если это не так, нам придётся ждать конца выполнения первой функции, чтобы запустилась вторая.
...
В языках программирования со статической типизацией объявление типов является встроенной возможностью. Вот некоторый пример Java-кода для иллюстрации:
...
Обратите внимание, как типизация встроена в само определение функции. Всё становится ещё туманнее, когда у вас появляются обобщения типов данных (*прим. пер., дженерики*):
...
Мы не сможем обнаружить ошибку до того, как всё приложение не упадёт, а это может случиться после нескольких месяцев прилежной работы на продакшене. Этого не произойдёт в Java, поскольку код там просто откажется компилироваться.
...
Ситуация приобретёт для вас больше смысла, когда вы увидите аннотацию с подразумевыемыми круглыми скобками:

```elm
add : Int -> (Int -> Int)
```

Здесь говорится, что `add` -- это функция, которая принимает *единственный* параметр типа `Int`, и затем возвращает функцию, принимающую *единственный* параметр `Int` и возвращающую `Int`.
...
Здесь говорится, что `doSomething` -- функция, которая принимает *единственный* параметр типа `String` и возвращает функцию, которая принимает *единственный* параметр типа `Int` и возвращает функцию, принимающую *единственный* параметр типа `String` и возвращающую `String`.
...
Здесь круглые скобки нужны, потому что `f` -- функция с типом `(a -> b)`, то есть такая функция, которая принимает отдельный параметр типа `a` и возвращает что-то с типом `b`.
...
Если вы видите `(a -> a)`, значит имеется ввиду, что входящий и выходящий типы ОБЯЗАНЫ быть одинаковыми. Не имеет значения, какие они именно, главное, чтобы они соответствовали друг другу.

Но в случае с `map` у нас есть `(a -> b)`. Это означает, что хоть функция МОЖЕТ возвращать разные типы, она также МОЖЕТ возвращать и одинаковые типы.

Но если тип `a` уже определён, он должен распространиться на всю остальную аннотацию (*прим. пер., сигнатуру*). Для примера, если `a` -- `Iny`, и `b` -- `String`, тогда аннотация будет подобна:
...
Тип `List Int` означает список, содержащий типы `Int`, и `List String` означает список, содержащий `String`. Если вы используете обобщения типов данных в Java или других языках, тогда такая концепция должна быть вам близка.
