...
Для этого выхожу из директории `dist` (*если мы еще находимся там*) так, чтобы оказаться в **корне приложения** `vuejs-test-app`. Затем выполняю подключение удалённого репозитория:
