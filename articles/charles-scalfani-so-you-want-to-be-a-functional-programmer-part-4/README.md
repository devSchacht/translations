...
Мы просто пишем функцию сложения, использующую два входных параметра, но принимающую один за раз. Функции ***каррирования*** (*прим. пер., «производные функции»*) позволяют нам сделать это.
...
Имейте в виду, что функция свёртки, `f`, принимает два параметра: текущий элемент массива `array` и счётчик `acc`. Она будет использовать эти параметры для обновления счётчика каждую новую итерацию. Значение счётчика в момент последней итерации будет возвращено из функции. 
