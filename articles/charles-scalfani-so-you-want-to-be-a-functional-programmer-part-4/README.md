...
Имейте в виду, что функция свёртки, `f`, принимает два параметра: текущий элемент массива `array` и аккумуятор `acc`. Она будет использовать эти параметры для обновления аккумуятора каждую новую итерацию. Значение аккумуятора в момент последней итерации будет возвращено из функции. 
