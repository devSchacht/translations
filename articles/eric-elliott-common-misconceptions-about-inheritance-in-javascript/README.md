...
- Именует тип объекта после конструктора, который вы сможете заметитть в консоли отладки. Вы увидите, например, `[Object Foo]` вместо `[Object object]`.
...
Кроме того, попытка заставить JS код вести себя как строго типизированный код может заблокировать использование ваших функций в дженериках, которые гораздо более пригодны для повторного использования и полезны.

<br>

`instanceof` лжёт.

Используйте Утиную тиипзацию.
...
Если после **профилирования своего приложения** вы обнаруживаете, что создание объекта действительно является узким местом, самый быстрый способ создать его - это не использовать ключевое словл `new` и классическое ОО. **Самый быстрый способ - использовать литералы объектов.** Вы можете сделать это с помощью циклов и добавить объекты в пулл, чтобы избежать удаления сборщиком мусора. Если и стоит отказаться от прототипного OO в пользу производительности, то стоит вообще отказаться от цепочки прототипов и наследования, чтобы получить литералы объектов.
...
Встроенные в JavaScript модули начинали использовать конструкторы, потому что [Брендану Эйку](https://twitter.com/brendaneich) было сказано сделать язык похожим на Java. JavaScript продолжает использовать конструкторы для самосогласованности. Было бы нелепо попытаться заменить всё на фабричные функции и запретить конструкторы.

<br>

Это не означает, что ваши API плох.
...
**jQuery остается самой популярной библиотекой JavaScript, когда-либо созданной. С ОГРОМНЫМ запасом. ОГРОМНЫМ.**
...
<blockquote>
<i>Приложение размером примерно в 100 тысяч строк кода начинает использовать Backbone. Несколько месяцев я производил отладку 6-ти уровневой иерархии, пытаясь найти баг. Проходил через каждую строку кода вверх по цепочке `super`. Нашёл и исправил ошибку в базовом классе верхнего уровня. Затем пришлось исправить множество дочерних классов, потому что они зависели от некорректного поведения базового класса. <b>В итоге часы разочарований вместо 5-тиминутного исправления бага.</b></i>
</blockquote>
...
для пожизненого доступа ко всем
