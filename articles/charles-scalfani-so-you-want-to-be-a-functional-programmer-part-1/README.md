...
Вспомните на что это было похоже – первый раз за рулём ***другой машины***? Было ли это похоже на ***самый первый раз*** за рулём машины вообще? Даже не близко. В первый раз всё было таким незнакомым. Конечно, мы сидлели до этого в машине, но только в роли пассажира. На этот раз мы оказывались в сидении водителя. Один на один со всеми рычагами, кнопками и педалями.
...
Обратите внимание, что `add(1, 2)` — в результате всегда даёт `3`. Не большой сюрприз, конечно, но это потому что функция чистая. Если бы функция `add` брала значение откуда-то снаружи, вы бы ***никогда*** не могли наверняка предсказать её поведение.
