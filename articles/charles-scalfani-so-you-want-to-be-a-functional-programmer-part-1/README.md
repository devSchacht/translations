...
И тот, кто учил Вас, говорил забыть изученное на уроках математики. Ведь в математике ***x*** никогда не мог равняться ***x + 1***.
...
Если вы не знакомы синтаксисом языка программирования ML, позвольте мне объяснить. ***addOneToSum*** -- это функция, принимающая 2 параметра: ***y*** и ***z***. 
...
Давайте подумаем, когда обычно мы хотим изменить переменную. Всего 2 главных причины, которые приходят на ум: многозначные изменения (например, изменение отдельного значения объекта или записи) и однозначные изменения (например, счётчики цикла).
...
К сожалению, такие примеры не очевидны в JavaScript, даже если Вы потратиили некоторое время на его изучение, по двум причинам. Во-первых, синтаксис JavaScript засорён, а во-вторых, Вы, вероятно, не привыкли думать рекурсивно. 
...
Я не объясняю здесь преимущества использования парадигмы неизменяемости, но Вы можете посмотреть фрагмент под названием ***Global Mutable State*** в статье [Why Programmers Need Limits](https://medium.com/@cscalfani/why-programmers-need-limits-3d96e1a0a6db), если хотите изучить эту тему.

Одно очевидное преимущество -- то, что если Вы имеете доступ к какому-либо значению в Вашей программме, это доступ только для чтения, а значит никто другой не может изменить это значение. Даже Вы сами. Вследствие, никаких случайных изменений. 

Также, если программа многопоточная, исполнение никакого потока не разрушит Вам планы. Поскольку значение -- константа и если поток захочет изменить его, ему придётся создать новое значение из старого.
