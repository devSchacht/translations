...
GraphQL неплох, он позволяет вам работать в декларативном стиле, давая (-вам) возможность выбирать информацию или операции, которые вам нужны.

Тем не менее, как и любой другой инструмент, это не [серебрянная пуля](https://ru.wikipedia.org/wiki/%D0%A1%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D1%8F%D0%BD%D0%BE%D0%B9_%D0%BF%D1%83%D0%BB%D0%B8_%D0%BD%D0%B5%D1%82).

В этом посте, я расскажу про пять причин по которым вы не должны использовать GraphQL как альтернативу REST архитектуре:
...
GraphQL альтернатива REST для разработки API, не замена.
...
И это не тяжело реализовать. Существует много [JSON API библиотек](http://jsonapi.org/implementations/) для многих языков программирования.
...
Конечно, бывают ситуцаии, при которых будет трудно использовать эти библиотки, и для таких случаев возможно будет лучше использовать GraphQL, который нативно поддерживать все эти возможности.
...
Вам будет необходимо распарсить сообщение, что бы узнать произошла ли ошибка, и возможно другие ошибки будут иметь слегка разный формат или добавлять некоторые произвольные поля.

Некоторые библиотеки, как Appolo client, помогают с [обработкой ошибок](https://www.apollographql.com/docs/react/features/error-handling.html), но не так же легко как в REST API.

Насчет загрузки файлов, эта функция не часть GraphQL спецификации, поэтому реализация зависит от вас. Некоторые варианты: 
...
## Легче использовать веб кэш с REST чем с GraphQl

Я хочу подчеркнуть веб-часть, кэширование на сетевом уровне, потому что вы, безусловно, можете реализовать кэш на уровне базы данных или на уровне клиента с использованием [кэша в памяти](https://www.apollographql.com/docs/react/advanced/caching.html) [Apollo Client](https://www.apollographql.com/client).
...
В GraphQL существует только одна конечная точка (в большинстве случаев конечная точка HTTP POST), куда отправляются все запросы. Поскольку каждый запрос может быть разным, сложнее использовать этот тип кэширования.
