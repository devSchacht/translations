# VuePress - создание системы документации
...
Документация для разработчика - это его хлеб насущный и очень часто случается так, что бывает трудно построить систему документации к разрабатываемым проектам. В этой статье будет рассмотрен вопрос создания статической документации, которая одновременно является одностраничным приложением ([SPA](https://en.wikipedia.org/wiki/Single-page_application)), в основе которого находится фреймворк [Vue](https://ru.vuejs.org/) и генератор статических сайтов [VuePress](https://vuepress.vuejs.org/).

## Что такое статические сайты

В общих словах можно сказать, что статические сайты - это страницы, которые хранятся точно также, как и предоставляются для показа пользователям. У таких сайтов нет обычного прямого взаимодействия типа клиент-сервер, за исключением возможного варианта с подключенными сторонними приложениями. В Интернете существует большое количество генераторов статических сайтов, которые различаются между собой отдельными характеристиками. У одних генераторов преимуществом является скорость работы и время загрузки; у других - гибкость настройки; третьи - обладают большим набором функциональности. Статические сайты предпочтительны в случаях, когда важна скорость работы страниц и низкая вероятность утечки секретной информации с сервера.

Особенностью VuePress как генератора статических сайтов является предустановленная тема оформления, которую можно использовать для создания документации тогда, когда для создания пользовательской темы нет времени. В этой статье будет использоваться тема по умолчанию.

## Почему именно VuePress?

Я хочу сказать - почему бы и не VuePress! По умолчанию VuePress генерирует все структурированные по папкам [markdown-файлы](https://en.wikipedia.org/wiki/Markdown) в HTML-файлы, которые будут использоваться в дальнейшем для показа пользователям. Генератор VuePress из коробки имеет в своем составе фреймворк [Vue](https://vuejs.org/), библиотеку [VueRouter](https://router.vuejs.org/ru/) для создания маршрутизации и систему [webpack](https://webpack.js.org/) для сборки проекта. Во время сборки все markdown-файлы преобразуются в Vue-шаблоны, в то время как все дополнительные файлы обрабатываются системой сборки [webpack](https://webpack.js.org/). Преобразование markdown-файлов в Vue-шаблоны предоставляет нам возможность использовать нативные Vue-скрипты в том виде, в каком они используются в обычных [Vue-компонентах](https://vuejs.org/v2/guide/single-file-components.html).

VuePress является системой, из коробки поддерживающей [SEO](https://en.wikipedia.org/wiki/Search_engine_optimization) и предоставляющей инструменты [Google Analytics](https://en.wikipedia.org/wiki/Google_Analytics) для анализа посещаемости страниц. Также VuePress имеет встроенную поддержку системы поиска по сайту при помощи индексации заголовков страниц и отображения их в процессе поиска.

VuePress имеет встроенную поддержку адаптивной разметки страниц, а также позволяет видоизменять или создавать свою собственную разметку. В рамках данной статьи мы создадим простую систему документации с поддержкой темы оформления по умолчанию; также мы воспользуемся такой возможностью, как **Vue in Markdown**, что позволит нам встроить простой счетчик внутрь markdown-файла.
...
Для создания проекта нам понадобятся установленные [Node.js](https://nodejs.org/en/) и менеджер пакетов [npm](https://en.wikipedia.org/wiki/Npm_(software)). Проверить их наличие и узнать их версии можно, набрав в терминале команду:
...
При создании папок в VuePress нужно принимать во внимание тот факт, что расположение файлов и директорий в структуре проекта имеет существенное значение. Каждый markdown-файл в любой из директорий компилируется в HTML-файл и маршрутом до этого файла будет являться родительская директория. Для начала, давайте создадим папку `docs`, внутри которой будут размещаться все будущие папки и конфигурационные файлы нашего проекта:
...
> Обратите внимание на тот факт, что перезагрузка сервера все же необходима в том случае, когда выполняется создание новых Vue-компонентов в момент работы запущенного сервера.

![VuePress](./images/vuepress-01.png)

> Обратите внимание на заголовок, созданный при помощи Vue-компонента

На момент когда запущен dev-сервер, основной целью является развертывание проекта. Для того, чтобы сделать вещи более прозрачными, давайте отредактируем файл `package.json` и включим в него команду для запуска dev-сервера аналогично команде для сборки проекта.
...
Заключительной частью процесса создания нашего проекта будет его размещение в Интернет на бесплатном хостинге [Netlify](https://www.netlify.com/). Сервис Netlify предоставляет отличную возможность бесшовной интеграции с [GitHub](https://github.com/) или другими системами версионного контроля для размещения готовых проектов на этом хостинге.
...
* **Шаг 2** - под зарегистрированной учетной записью зайти на сервис Netlify и выбрать опцию "New site from Git". Выберите систему контроля версий GitHub и в ней - репозиторий с проектом.
...
По указанной ниже ссылке располагается размещенный на Netlify проект, который мы создавали в этой статье - [Scotch VuePress](https://scotch-vuepress.netlify.com/).
