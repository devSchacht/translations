# Как я портировал bcrypt для нового N-API
...
Они являются мостом между привычным на языком программирования JavaScript и родной средой, полностью написанной на C/C++.

Они позволяют нам называть функции и методы C/C++ непосредственно из JavaScript.

![](https://cdn-images-1.medium.com/max/2000/1*oJ1O-rd1CwjJhlzsuFQrYA.png)
 *Нативные модули Node.js*

Одной из наиболее важных экспериментальных функций, объявленных с Node.js 8, была поддержка [N-API](https://nodejs.org/dist/latest/docs/api/n-api.html), которая направлена на снижение затрат на обслуживание нативных модулей Node.js.

> Этот API не зависит от среды исполнения JavaScript (V8) и поддерживается как часть самого Node.js.
> 
> Этот API будет стабильным на уровне бинарного интерфейса приложений (**Application Binary Interface** или **ABI**) между версиями Node.js. Он предназначен для изоляции модулей от изменений в движке JavaScript и позволяет использовать нативные модули различными версиями Node.js без перекомпиляции.
> 
> Такие модули создаются/упаковываются с использованием того же самого подхода и инструментов, описанных в разделе [C++ Addons](https://nodejs.org/dist/latest/docs/api/addons.html). Единственное отличие — это набор API, которые используются нативным кодом. Вместо использования API V8 или [Native Abstractions](https://github.com/nodejs/nan) используются функции, доступные в N-API.

Как вы можете понять из документации по N-API, **оно не зависит от версии движка JavaScript, и гарантируется совместимость API и ABI между различными версиями Node.js**. Поэтому, если вы переключитесь на другую версию Node.js, вы не должны переустанавливать и перекомпилировать нативные модули.
...
Я обнаружил, что есть обёртка над N-API для C++, называемая [**node-addon-api**](https://github.com/nodejs/node-addon-api), поэтому я просто начал экспериментировать с ней, и через 4 или 5 дней своего свободного времени я портировал [**bcrypt**](https://www.npmjs.com/package/bcrypt) на N-API.
