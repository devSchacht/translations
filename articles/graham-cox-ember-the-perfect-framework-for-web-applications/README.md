...
Не мало возможностей Ember.js связано с его [интерфейсом командной строки или CLI](https://ru.wikipedia.org/wiki/%D0%98%D0%BD%D1%82%D0%B5%D1%80%D1%84%D0%B5%D0%B9%D1%81_%D0%BA%D0%BE%D0%BC%D0%B0%D0%BD%D0%B4%D0%BD%D0%BE%D0%B9_%D1%81%D1%82%D1%80%D0%BE%D0%BA%D0%B8). Этот инструмент, известный как ember-cli, управляет большой частью процесса разработки, начиная от создания приложения, добавления различного функционала во время разработки и заканчивая запуском тестов и деплоем на продакшен.

Практически все во время разработки Ember.js приложения будет в какой-то степени связано с этим инструментом, поэтому важно понимать как им пользоваться. Мы будет использовать его при создании нашего приложения.

Первым делом нужно убедиться, что ember-cli установлен и актуален. Установливаем c помощью npm: 
...
С установленным ember-cli мы готовы создать наше приложение. В первый раз мы будем использовать ember-cli для того чтобы создать всю структуру приложения и сделать первоначальные настройки.
...
Запуск сервера Ember приложения для целец разработки делается **снова** с помощью ember-cli:
...
Все готово. Приложение запущено по адресу http://localhost:4200 и выглядит следующим образом:

![](https://dab1nmslvvntp.cloudfront.net/wp-content/uploads/2017/10/1507570414first-view.png)

Также запустился [LiveReload](http://livereload.com/) сервер, который автоматически наблюдает за изменениями в файловой системе. Это значит, что любое изменения в коде приведут к автоматической перезагрузке нашего приложения в браузере. А изменения картинок или css применятся даже без перезагрузки.

Начальная страница подсказываем нам что делать. Давайте изменим ее и посмотрим что произойдет. Мы собираемся изменить файл `app/templates/application.hbs`, чтобы он выглядел так:
...
Это говорит нам о том, что сервер заметил наши изменения в шаблоне, пересобрал и перезапустил все каких-либо действий с нашей стороны.
...
Может не очень впечатляюще, но мы получили это почти без училий с нашей стороны.
...
Заметьте, что в выводе консоли упоминается [PhantomJS](http://phantomjs.org/). Это потому, что Ember имеет полную поддержку запуска интеграционных тестов в браузере и по умолчанию используется [безголовый браузер](https://en.wikipedia.org/wiki/Headless_browser) PhantomJS. Вы можете настроить запуск тестов в любом браузере по вашему желанию. Например при настройке [непрерывной интеграции](https://ru.wikipedia.org/wiki/%D0%9D%D0%B5%D0%BF%D1%80%D0%B5%D1%80%D1%8B%D0%B2%D0%BD%D0%B0%D1%8F_%D0%B8%D0%BD%D1%82%D0%B5%D0%B3%D1%80%D0%B0%D1%86%D0%B8%D1%8F) (CI) будет полезно воспользоваться этим, чтобы удостовериться, что приложение корректно работает во всех поддерживаемых вами браузерах.

## Как устроено Ember.js приложение

Перед тем как мы приступим к написанию кода, давайте изучим структуру нашего приложения в файловой системе. Команда `ember new`, используемая нами ранее, создает целую директорию с файлами и папками. Понимание структуры этой директории необходимо для эффективной работы с инструментом и создания удивительных проектов.

В корне директории вы можете увидеть следующие файлы и папки:

* **README.md** – стандартный файл описывающий фаше приложение.
* **package.json** – стандартный файл пакетного менеджера npm, также описывающий ваше приложение, но с точки зрения зависимостей. В нем описаны зависимости вашего приложения и их версии, для того чтобы они устанавливались корректно.
* **ember-cli-build.js** – конфигурационный файл для ember-cli.
* **testem.js** – конфигурационный файл для тестирующего фреймворка testem. Он позволяет, среди прочего, определеть в каких браузерах будут запускаться тесты. 
* **app/** – здесь хранится логика приложения. Многое проиходит в этой папке и мы рассмотрим это ниже.
* **config/** – конфигурация нашего приложения
  * **config/targets.js** – содержит список поддерживаемых браузеров. Это необходимо для Babel, чтобы он транспилировал код таким образом, чтобы он работал во всех необходимым вам браузерах.
  * **config/environment.js** – содержит основные необходимые настройки приложения, отличающиеся в различных окружениях.
...
Добавление подобного стороннего контента может быть осущественно несколькими способами:
...
В Ember приложении каждая посещаемая страница доступна с помощью Роута. Существует прямая связь между адресной строкой (URL) в браузере и роутом в приложении. 
...
Давайте посмотрим на это в действии. Пока что мы хотим создать довольно простую страницу, позволяющую нам получить число, после броска костей. Для этого обновим файл `app/templates/roll.hbs`:
...
Теперь нам нужна возможность попадать на эту страницу с главной страницы. Ember позволяет сделать это очень просто с помощью хелпера `link-to`, который принимает первым аргументом имя роута и рендерится в разметку, позволящую нашему пользователю попасть а нужный роут.
...
Если на данном этапе вы попробовали потестировать наше приложение, то могли заметить одну проблему. Переход по ссылке в роут `roll` работает корректно, но метки элементов форм не выстраиваются правильно. Это происходит потому что Materialize нужно использовать JavaScript, чтобы поставить метки на свои места после того как они будут отрендерены. Но динамический роутинг предпологает, что страница не будет перезагружена. Таким образом, у нас пока что нет места, где можно инициализировать этот JavaScript код. 

На помощь приходят компоненты. Компоненты это частички интерфейса пользователя, которые имеют свой жизненный цикл с которым мы можем взаимодействовать и в который мы может встроить необходимый нам JavaScript код. Также они используются для создания переиспользуемых элементов интерфейса, но к этому мы еще вернемся.

Пока что мы собираемся создать компонент, представляющий собой форму для броска костей. Как обычно, создание компоненты тоже можно осуществить с помощью ember-cli:
...
* **tests/integration/components/roll-dice-test.js** – Тест, чтобы удостовериться, что компонет работает правильно

Мы перенесем всю разметку из роута `roll` в компонент, что не повлияет на работу приложения в целом, но позволит нам использовать всю силу компонент.
...
## Жизненные цикл компонента

Компоненты в Ember имеют заданный жизненный цикл, которому они следуют, с множеством хуков вызываемых на различных стадиях. Мы используем хук `didRender`, который вызовится после того как компонент будет отрендерен – в первый раз или в любые последующие – чтобы попросить Materialize обновить метки элементов форм.
...
* Как строку в ковычках – значением будет эта строка
* Как строку без ковычек – в этом случае значение будет взято из одноименного поля в классе компоненты, но компонента никогда не обновится
...
Все вышеизложенное означает, что теперь мы можем обращаться к добавленным в компонент полям, как к значениям полей ввода, а все остальное сделает Ember.

## Экшены компонент

Следующим шагом мы хотим добавить компоненту интерактивности. Например, хорошо бы обрабатывать нажатие кнопки "Бросить кости". Для этого в Ember есть экшены. Это методы, описаные в классе компонента, которые могут быть подключены в шаблон компонента. Обычно методы описывают в специальном объекте компонента `actions`.

Добавим экшен в нам компонент `app/components/roll-dice.js`:
...
Вы можете заметить, что мы ссылаемся на поля, которые мы ранее объявили в классе, чтобы иметь доступ к значениям полей ввода. Здесь нет никакого взаимодействия с DOM – мы оперирует только JavaScript переменными.

Осталось подключить наш экшен. В шаблоне, нам нужно сказать тэгу формы, что ему нужно вызвать `triggerRoll` экшен, когда случится событие `onsubmit`. Это делается добавлением всего одного атрибута с испльзованием `action` хелпера. В шаблоне `app/templates/components/roll-dice.hbs` это  выглядит так:

```
<form class="col s12" onsubmit={{action 'triggerRoll'}}>
```

Заполнив поля формы и нажав на кнопку, мы увидем диалоговое окно с результатом нашего ввода.

![](https://dab1nmslvvntp.cloudfront.net/wp-content/uploads/2017/10/1507570432alert.png)

## Взаимодействие с сервером

Следующим шагом будет написание логики настоящего броска кости. Это требует взаимодействие с сервером, так как сервер ответственен за запоминание результата броска кости.

Мы хотим достичь следующего:

* Пользователь определяет параметры для броска костей
* Пользователь нажимает на кнопку "Бросить кости"
* Приложение выполняет логику броска кости отправляет параметры совершенного броска на сервер
...
И перепишем логику экшена в компоненте. Теперь мы хотим вызвать другой экшен в нашей компоненте, передав параметры броска в его аргументы. Это делается с помощью метода `sendAction`, доступного в классе компонента.
...
И вновь, все сделанное нами никак не повлияло на работу приложения, но зато все части в правильных местах.

## Сохранение в хранилище

Прежде чем мы сохраним данные в хранилище, нам необходимо определить модель, представляющую эти данные. Используя наш надежный инструмент ember-cli, создадим структуру модели и затем заполним ее.
...
Теперь мы можем заполнить модель `app/models/roll.js` необходимимы для представления наших данных атрибутами:
...
`DS.attr` вызывается чтобы определить атрибут модели заданного типа. Эти типы называются преобразователями(`transform`) в Ember. Варианты преобразователей по умолчанию: `"string"`, `"number"`, `"data"` или `"boolean"`. Но вы всегда можете добавить свой при необходимости.

Используем эту модель для создания броска кости в хранилище и передачу на ьэкенд сохранения. Для этого нам нужно получить доступ к хранилищу в классе роута `app/routes/roll.js`:
...
Наш роут напрямую обращается в хранилище и используя метод `findAll` загружает все сохраненные в нем броски кости. Затем мы предоставляем эти данные в шаблон используя хук роута `model`.
...
В шаблоне мы имеет доступ в модели, возвращанной роутом и итерируясь по ней создаем таблицу, которая выглядит так:

![](https://dab1nmslvvntp.cloudfront.net/wp-content/uploads/2017/10/1507570398history.png)

## Заключение

На данный момент, благодаря относительно небольшим усилиям, мы создали приложение, которое позволяет нам бросать кости и просматривать историю бросков. Это включает в себя связывание данных из нашей формы, сохранение данных в хранилище и чтение их из него, шаблоны для показа всех страниц и полноценный роутинг связанный с URL. Это приложение может быть создано с нуля за час.

Использование Ember может очень сильно повысить эффективность разработки фронтенда. В противоположность библиотекам, вроде React, Ember дает вам весь необходимый функционал без дополнительных усилий. Использование ember-cli и настроенных процессов сборки приложения выводит его на следующий уровень, делая процесс невероятно простым и безболезненным от начала и до конца. Добавим сюда поддержку [сообщества](https://www.emberjs.com/community/) и практически не остается задач, которые не могли бы быть решены.

К сожалению, может быть сложно использовать Ember вместе с уже существующим проектом. Это работает прекрасно для старта нового проекта. Также Ember работает из коробки только с несколькоми вариантами API бэкенда и если ваш бэкенд не соответствует им, то вы можете потратить много времени либо переписывая бекенд, либо настраивая Ember на работу с вашим API. 
