...
*Подключает React-компонент к Redux*, позволяя компоненту использовать стор верхнего уровня, при этом отпадает необходимость получать стор в виде пропсов от родителя.([взято отсюда](https://medium.com/@firasd/quick-start-tutorial-using-redux-in-react-apps-89b142d6c5c1)). Условно работу **connect(...)** можно разбить на следующие стадии:

1. **_Передать стор внутрь корневого компонента_**
