...
После анализа тысячи шаблонов, мне пришлось приготовить чашку чая, чтобы успокоиться. Потому что ни одни из шаблонов не оказался простым в освоении. Так что мне придется начать изучение шаблонов с нуля. Эта статья представляет собой заметку, где я поясняюю, как разбить шаблон на понятные части, шаг за шагом. Ниже я представлю 5 причин создания такого большого количества изоморфных шаблонов:
...
Выше представлена структура работы приложения. Приложения используется Express, который работает только с одним роутом, с помощью функции *res.sendFile* отображая index.html в браузере. Особое внимание следует уделить части, обведенной голубым контуром. Там показано взаимодействие react'а, redux'а, корневого компонента, стора и редюсера.
