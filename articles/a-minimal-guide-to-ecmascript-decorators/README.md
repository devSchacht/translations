...
спецификаций языка ECMAScript. Члены этой команды являются частью ECMA International, разработчиков браузеров и компаний,
заинтересованным вебом в целом.
...
В настоящее время (*июнь 2018*) __декораторы__ находятся на **втором этапе**, и у нас есть Babel-плагин для транспиляции декораторов `babel-plugin-transform-decorators-legacy`. На втором этапе, поскольку синтаксис возможности подвержен изменениям, на данный момент не рекомендуется использовать в продакшене. В любом случае, декораторы прекрасны и очень полезны для достижения ускорения процесса разработки.
...
Для понимания декораторов нужно сначала разобраться, что такое __дескриптор свойства__ (property descriptor) свойства объекта в JavaScript. Дескриптор свойства представляет собой набор правил для свойства объекта, как например, является ли свойство доступным для записи (__writable__) или перечислимым (__enumerable__). Когда мы создаём простой объект и добавляем некоторые свойства, у каждого из свойств есть дескриптор свойства по умолчанию.
