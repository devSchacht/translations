...
```
Копирование текста в буфер обмена - проблема, решаемая по-разному. Современным браузерам будет достаточно вышеописанного примера. Это довольно "чистое" решение, оно не полагается на элементы DOM.

*Обратите внимание, что функция* ```copyTextToClipboard``` *является асинхронной, поскольку функция* ```writeText``` *возвращает* ```Promise```.   

Однако при необходимости поддерживать старые браузеры, такие как Internet Explorer, вам понадобится следующий подход:
```
// HTML
...
```
Это решение полагается на поле ввода, в отличие от предыдущего, основанного на ```Promise```.
******
...
Вычисление количества дней между двумя датами - одна из тех вещей, которые вы сделаете более одного раза, если регулярно программируете на JavaScript. Чтобы не выяснять способ решения каждый раз, предлагаю вам использовать функцию, уже сделавшую тяжелую работу за вас.
```
...
// Result: 373
```
Из-за использования ```Math.abs()``` порядок аргументов-дат не имеет значения.
******
#### Вот и всё
Я надеюсь, что теперь, дочитав список до конца, вы узнали для себя кое-что новое. Также я надеюсь, что вы сможете применить некоторые из этих приемов на практике. Если вы знаете отличное однострочное выражение на JavaScript, которого нет в этом списке, сообщите [мне](https://medium.com/@daaaan) [прим. переводчика: напишите комментарий под статьей [15 Helpful JavaScript One-Liners](https://medium.com/javascript-in-plain-english/15-helpful-javascript-one-liners-946e1d1a1653)]. Я был бы рад увидеть еще больше замечательных однострочников.
