# Реактивные интерфейсы на нативном JS - Часть 1: Чистый функциональный стиль
...
Вы возможно уже поняли, почему такой способ эффективен для нас. Ранее мы определили концепцию значения-функции другого значения. `Array.map()` позволяет нам получить целый массив, для которого каждый элемент является функцией соответствующего элемента в другом массиве. Допустим, у нас есть массив сообщений в блогах, которые мы хотим отобразить:
...
К счастью, наш сайт - это всего лишь блог. Содержание ощутимо меняется только при просмотре другого поста в блоге. Нет множества взаимодействий для отслеживания, нам не нужно оптимизировать операции с DOM. Когда мы загружаем новый пост, мы можем просто удалить DOM и перестроить его.
...
Теперь, когда мы загружаем данные поста, просто вызываем `update()`, и отображается HTML. Если бы наше приложение было достаточно сложным, настолько что его нужно было бы перерисовывать часто - например, пару раз в секунду в определенных ситуациях - оно быстро начало бы тормозить. Можно написать сложную логику, чтобы определять, какие части страницы действительно нуждаются в обновлении, учитывая конкретное изменение данных, и обновлять только  их, но это как раз тот случай, когда надо просто использовать фреймворк.
