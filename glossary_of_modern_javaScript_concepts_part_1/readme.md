...
Теперь, когда у нас есть объединенный поток времени и движения, мы создаем `подписку` на observable `source$`, чтобы мы могли реагировать на него. В обратном вызове `onNext` мы проверяем, является ли значение числом или нет. Если является, мы вызываем функцию `createTimeset()`. Если это объект координат, вызываем `addPoint()`. В обратных вызовах `onError` и `onCompleted` мы просто логируем информацию.
