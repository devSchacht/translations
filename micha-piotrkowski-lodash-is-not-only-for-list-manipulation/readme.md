...
> **_Заметка для продвинутых_**: порядок параметров в `_.mergeWith(customizer, object, source)` немного неудачный, посколько он принимается параметр с данными (`object`) вторым и, при этом, не последним параметром. Иначе мы могли бы в полной мере воспользоваться каррированием и определить функцию `patch` просто как:
