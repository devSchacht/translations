...
*`$ npx -p cowsay -p lolcatjs -c ‘echo “$npm_package_name@$npm_package_version” | cowsay | lolcatjs’` устанавливает как cowsay, так и lolcatjs, и дает скрипту доступ к множеству переменных `$npm_` из запущенных скриптов.*
